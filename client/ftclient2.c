...
//	messageSize = atoi(buffer);
//	printf("size of message is %d\n", messageSize);
	

//	while (count < messageSize) {
//		charsRead = recv(socketFDData, buffer + count, messageSize - count, 0);
//		count += charsRead;
//	};
	close(socketFD);
...
//	if (checkForFile(argv[4])) {
//		appendCopy(
//	};

	//FILE* outputFilePtr = fopen(argv[4], "w");
	//fprintf(outputFilePtr, buffer);
